# Source inventory — what is on disk, and what is missing
...
| Sira — Ibn Ishaq / Guillaume | `sira-guillaume--guillaumeathelifeofmuhammad.txt` | The good copy (30,883 lines) |
...
| File | Problem | Impact |
|---|---|---|
| `sira-guillaume--thelifeofmohammedguillaume.txt` | Extraction yielded **1 line**. The 147 MB source PDF has no usable text layer. | None on coverage — the *other* Guillaume copy is intact and covers the same translation. This is a dead file; a candidate to delete. |
...
| `tabari--the-history-of-al-tabari.txt` | This is the **Volume XL Index only**, not narrative text. | No ruler/date narrative from it. See the Tabari gap below. |
...
### 4. Tabari narrative volumes — **missing for the imams' period**
- **Work:** *The History of al-Tabari* (SUNY), volumes covering **40–260 AH**, incl. **vol. 19 (Karbala)** and the Abbasid volumes.
- **Blocks:** the ruler bullet in all fourteen fact panels; envelopes 04, 08, 11, 13, 14.
- **Status:** on disk are **vol. 8** (`tabarivolume08` — early Medina, outside the needed range) and the **vol. 40 Index** (no narrative). None of the 40–260 AH range is present.
...
Qarashi lives (Envelopes 05/07/11/13), and the Tabari 40–260 AH volumes (the
ruler bullets).** Two on-disk files are degraded — the second Guillaume copy
(dead, deletable) and al-Kafi (Part 1 only).
