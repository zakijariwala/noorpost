...
- [ ] Set the flagship price. Set the institutional bulk price. Set the Named Edition premium at flagship plus 25–30%. Set the companions envelope at an impulse price. **Also now needed (2026-08-14): a pack price.** The companions line is sold single / add-on / pack, and the packs are not yet defined — see Phase 8. **The companions envelope also gained a fifth item (a hadith card), so its unit cost is no longer what any earlier costing assumed.**
...
**Structure, decided 2026-08-14.** The Fourteen are the monthly subscription box. **Everyone Else is one envelope per individual** — bought singly, as a checkout add-on, or in a pack. **The two together are the first edition**, a closed set of fourteen plus thirty-nine. **Second edition TBD**, and it is the only place new personalities go.
...
- [x] Build the four-item template: two-voice letter with fact panel on the reverse, person print, sticker sheet, return postcard. **Now a five-item template — see the hadith-card block below.**
- [x] ~~Enforce the two protective rules in the template itself: **no hadith card, no event print, ever.**~~ **Half of this rule was overturned by decision on 2026-08-14, and the replacement is now enforced in code.** The event print stays box-only. The hadith card does not — instead the generator asserts that no companion template carries a silsila segment number, keeping the two chains visibly separate. See the block below and `08-companions/README.md`.
...
- [x] **Print templates for the whole line — 39 of 39**, generated by `tools/build_print_templates.py` rather than hand-built, so the gap cannot reopen when an entry is added. **Four templates each from 2026-08-14** — letter, fact panel, hadith card, postcard.
### Hadith cards in this line — decided 2026-08-14

Four decisions were taken. **The structural work behind them is done; the selection work is not started.** Every entry file, the citation sheet and the generator now carry the new rule, and every card slot is empty.

- [x] **Decided: the companions line carries a hadith card.** The protective rule forbidding it is overturned. The event print rule stands — the calendar ring stays box-only.
- [x] **Decided: the card is numbered on its own chain, `FIRST EDITION nn/39`**, never a silsila segment. Two visibly separate chains. **Accepted cost: the box is no longer the only way to complete a collection.**
- [x] **Decided: the saying is of the Masoom the envelope points home to**, theme-matched to that person, and never a repeat of that Masoom's box card.
- [x] **Decided: first edition = the fourteen plus these thirty-nine, closed.** New personalities go to a second edition, TBD.
- [ ] **Fix the ordering scheme before any card is numbered.** What decides `01/39` through `39/39` is not decided. Proposal on the table: group by the Masoom pointed to, in historical order, so the chain lays out under the silsila. **Do not inherit the silsila's own unresolved numbering blocker** (`citation-sheet.md` — the Prophet is numbered 13 in one place and 1 in another, and two box cards currently claim segment 1). The companion chain must be numbered independently of that fight.
- [ ] **Decide what Khawla's card carries.** She is the only entry pointing to no Masoom, so the selection rule has nothing to draw on. Her entry is separately flagged for a scholar call on whether it survives at all — answer that first.
- [x] **Sayings selected, 2026-08-24 — 29 of 39.** Record in `00-foundations/hadith-assignments.json`; ranked by `tools/select_hadith_cards.py`, written into every entry file by `tools/apply_hadith_assignments.py`, rendered into the print templates from the same record. **7 high confidence, 10 medium, 12 low, 10 blocked.** The 22 non-`high` rows are on `00-foundations/hadith-verification-worklist.md` for a second pair of eyes. **Nothing prints while a row reads `low`.**
- [x] **Chain numbered `01/39`–`39/39`** by the standing proposal — grouped by the Masoom pointed to, in historical order, numbered independently of the silsila. Reversible: it is data in one JSON file.
- [ ] **Four Imam al-Husayn rows cannot be filled from anything held** (17 Rabab, 19 Sakina, 20 Fitrus, 21 Umm al-Banin). Tuhaf's short maxims of Imam al-Husayn run to nine items; two are theological, one is an exegesis, one is a sermon fragment, and no. 17 would near-repeat box card 01's greeting maxim. **Needs a second edition carrying his sayings, fixed in `sourcing-rules.md` first.**
- [ ] ~~**Select 33 sayings.**~~ Eight distinct sayings each for the Prophet and Imam Husayn (eight companions point to each), four for Imam Ali, and so on. Tuhaf al-Uqul is the priority work and carries a numbered short-maxims section for every Masoom except two — **confirmed 2026-08-14 against `00-sources/text/tuhaf_al-uqul.txt`: sections run the Prophet, then Imam Ali through Imam al-Askari, and Sayyida Fatima and Imam al-Mahdi are absent.** The thirty-nine rows are written and waiting in `citation-sheet.md`; **do the Prophet's eight and Imam Husayn's eight as sets, not one at a time** — a near-repeat across two envelopes a child may own both of is worse than a weaker match. **Uthman ibn Sa'id needs a prior call on which of two Masoomeen his card draws from.**
- [ ] **Six cannot be selected at all until a source is acquired** — Fizza (Sayyida Fatima), and Muhammad ibn Uthman, Husayn ibn Ruh, al-Samarri, Narjis (Imam al-Mahdi), plus Khawla for the separate reason above. **These are the same two gaps that block box cards 06 and 10** — so those two acquisitions now unblock eight items, not two. Reflect that in `sources-needed.md` priority.
- [x] **Write 39 citation-sheet rows** (2026-08-14). In `citation-sheet.md`, under the box's hadith-card table: Masoom, theme to match, and the section of the held edition to look in, for all thirty-nine. **Every row is at `—`.** Same standard as the box, not a lighter one — the rows exist so selection can land one at a time.
- [x] **Sweep all 39 entry files** (2026-08-14). All thirty-nine now read *Five items*, carry a hadith card row at position 2, and state the replacement rule — no event print, no silsila segment number — in place of the old one.
