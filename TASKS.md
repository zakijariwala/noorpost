...
- [x] ~~Enforce the two protective rules in the template itself: **no hadith card, no event print, ever.**~~ **Half of this rule was overturned by decision on 2026-08-14.** The event print stays box-only. The hadith card does not. See the block below and `08-companions/README.md`.
...
### Hadith cards in this line — decided 2026-08-14, nothing built

Three decisions were taken and **no content work followed them.** Nothing below is started.
...
- [ ] **Select 33 sayings.** Eight distinct sayings each for the Prophet and Imam Husayn (eight companions point to each), four for Imam Ali, and so on. Tuhaf al-Uqul is the priority work and carries a numbered short-maxims section for every Masoom except two.
...
- [ ] **Write 39 citation-sheet rows.** Same standard as the box, not a lighter one.
- [ ] **Sweep all 39 entry files.** Every one still prints *"No hadith card. No event print. Rule enforced in the template"* in its Items block — half wrong in all thirty-nine — and every Items table still says four items.
- [ ] **Replace the rule assertion in `tools/build_print_templates.py`** and add a card template. The generator asserts the old rule; the new check is narrower — no silsila segment number, no event print.
