# Everyone Else — Husayn ibn Ruh al-Nawbakhti

**Points home:** *Husayn served the third of the four terms, and was so careful with what he said that even his enemies could not catch him saying too much.* · Dateless · Four items

---

## Letter — *The Man Who Answered Without Answering*

<!-- LETTER START -->
● The third man was nothing like the first two. No stall, no oil, no cover at all. He came from a family everyone in Baghdad already knew, and everyone already knew his name.

○ Isn't that backwards?

● It should have ruined him. The first two hid behind an ordinary trade. He could not hide behind anything, so he did the other thing instead.

○ Which was?

● He learned to answer everything and give away nothing.

○ You can do both?

● He could. A government man once questioned him for a long stretch — circling back, rephrasing, trying to get a name out of him — and went away with nothing he could use. By some accounts he went away pleased with how it had gone.

○ Did they ever catch him?

● Never with anything in his hands. They jailed him more than once anyway, on suspicion alone, held him, learned nothing, and let him out again exactly as suspicious as when they took him in.

○ What was he protecting?

● Names. Ordinary people, in a dozen cities, who had trusted him with letters and with money. One careless sentence in one long conversation and somebody he had never met would have paid for it.

○ How do you even learn that?

● Slowly, and by deciding beforehand. You work out what you will not say before you are ever asked, because nobody makes that decision well in the middle of being questioned. Then you answer generously about everything else, so the silence has somewhere to hide.

○ Did he pick the next one too?

● He did — Ali ibn Muhammad al-Samarri, the fourth and last. Three handovers by then, each man naming the next while he was still alive to be argued with.

○ Could you guard a name for years? Say if you could.

● Very few people can. Most of us give a thing away tired, or cornered, or wanting to be liked. He was all three at times and still did not.

●○ He answered every question they had. He told them nothing.
<!-- LETTER END -->

**One new thing for the adult:** most people who know the four deputies by number don't know that one of them was famous, in his own time, specifically for how he spoke under pressure.

**How much room he had:** watched more closely than either man before him, and trusted with exactly the same secrets anyway.

---

## Fact panel — letter reverse

### HUSAYN IBN RUH AL-NAWBAKHTI

● **His background.** Born into the Nawbakhti family, well known in Baghdad for scholarship — unlike the first two deputies, he was a public figure before he ever took the post.

● **Who he served.** Third of the four trusted agents (*sufara'*) of Imam al-Mahdi, named by Muhammad ibn Uthman before his death.

● **What he was known for.** Extraordinary discretion under direct questioning — reports describe interrogators coming away satisfied without having learned anything usable.

● **What it cost him.** Imprisoned more than once during his term, by rulers who suspected the role without ever being able to prove it.

● **Why he is in this envelope.** *Husayn served the third of the four terms, and was so careful with what he said that even his enemies could not catch him saying too much.*

He died still holding the post, having named Ali ibn Muhammad al-Samarri as the fourth.

---

## Items — artwork pending

| # | Item | Spec | State |
|---|---|---|---|
| 1 | Letter + fact panel | Written | ✓ |
| 2 | Person print | **A portrait. Faces allowed in this line.** A composed man seated across a table from an unseen questioner, hands still, giving nothing away. | Pending |
| 3 | Sticker sheet | A closed door, a locked chest, a sealed letter, small repeatable marks | Pending |
| 4 | Return postcard | Fixed wording per `standard-lines.md` §4 | Pending |

**No hadith card. No event print.** Rule enforced in the template.

**Exterior:** no month cancellation — this line is dateless. Plain seal, name area, *He told them nothing at all* printed small.

---

## Spec check

Measured with the command in `03-content/spec-check.md`, not estimated.

| Rule | Required | This letter |
|---|---|---|
| L5 | 330–370 words | **332** ✓ |
| L6 | 6–9 child lines | **9** ✓ |
| L7 | Every child line under 15 words | longest is 11 ✓ |
| L8 | One child line demands a response | the "Say…" line ✓ |
| L9 | Last line read together | ●○ ✓ |
| L14 | Never builds to the death | ✓ |
| L16 | No violence in scene | ✓ |

**Child-line register:** average 5.2 words. The established letters run 4.0–6.3; this sits inside that band deliberately — the 15-word cap is a ceiling, not a target.

**Child-word test:** every word in every child line sits inside Dolch + Fry 1000 or the madrasa list. Proper nouns (names, places) appear only where the child is naming a thing in the act of asking about it, which is the allowance used in `01-pilot/envelope-03/letter.md`. Nothing else flagged.

## Notes

**This is the first of the four deputies whose story is about a skill rather than a trade.** Uthman and Muhammad ibn Uthman hid behind an ordinary occupation; Husayn ibn Ruh could not — he was already known — so the letter's "one new thing" has to be about *how* he protected the network rather than *where* he hid it. That distinction should stay sharp through any future edit.

**No occultation theology anywhere in this letter**, per the same rule the other three deputy entries and `envelope-10.md` hold to. The letter stays on the working discipline of the post — nothing about why the arrangement existed or what it means.

**The interrogation scene must stay a fact, not a dramatisation.** No invented dialogue for the questioner, no scene-setting beyond what is needed — the letter names that it happened and what the result was, in the same restrained register as the rest of this line.

## Open

- [ ] Verify the length of Husayn ibn Ruh's term and the span of years against a fixed edition
- [ ] Verify the imprisonment accounts and their circumstances specifically
- [ ] Check this letter's claims sit consistently with `muhammad-ibn-uthman.md`, `al-samarri.md`, and `envelope-10.md` — same names, same order
- [ ] Scholar review — **occultation-adjacent content, check rule Q3 specifically**
